class Transaction < ActiveRecord::Base
  ...
  scope :last_six_months, -> {where('transactions.created_at >= ? AND transactions.created_at <= ?', (Time.now - 6.months).end_of_day, Time.now.end_of_day)}
  ...
  def transfer_or_account_name
    if self.transfer.present?
      self.transaction_type == 3 ? self.transfer.from_bank_account.bank_account.name : self.transfer.to_bank_account.bank_account.name
    else
